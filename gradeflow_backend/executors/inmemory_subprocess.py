...
class InMemorySubprocessJobExecutor(GradingJobExecutor):
    def __init__(
        self,
        *,
    ...
    def submit(self, spec: GradingJobSpec, callback_url: str) -> str:
        ...
        with self._lock:
            ...
            # Notify one or more workers that a job is available
            self._cv.notify()
        return job_id
    ...
    def _pop_next_locked(self) -> _Job | None:
        """
        Pop next job with preview priority. Caller must hold self._lock.
        """
        ...
        while True:
            with self._lock:
                ...
                while not self._stop_event.is_set():
                    ...
                    if job is not None:
                        break
                    # If no job, wait with timeout to allow periodic wake-ups
    ...
    def _execute(self, job: _Job) -> GradingJobResult:
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            qset_yaml = tmp / "question_set.yaml"
            rubric_yaml = tmp / "rubric.yaml"
            out_yaml = tmp / "graded.yaml"
            ...
            if not out_yaml.exists():
                raise RuntimeError("CLI did not produce the expected output file")

            items_raw = out_yaml.read_text(encoding="utf-8")
            items: list[Any] = yaml.safe_load(items_raw) or []

            graded = [GradedSubmission.model_validate(obj) for obj in items]
...
def create_executor() -> GradingJobExecutor:
    timeout_s = int(os.getenv("JOB_TIMEOUT_S", str(DEFAULT_TIMEOUT_S)))
    poll_interval_s = float(os.getenv("JOB_POLL_INTERVAL_S", str(DEFAULT_POLL_INTERVAL_S)))
    num_workers = int(os.getenv("JOB_NUM_WORKERS", "1"))
    executor = InMemorySubprocessJobExecutor(
        timeout_s=timeout_s,
